...
class TesterOne(object):
    ...
    def generate_fake(self, segmap, styimg=None):
        # Load the segmentation map
        # ~ segmap = self.preprocess_image(segmap_path, num_channels=1,
        segmap = self.preprocess_image(segmap, num_channels=1,
        ...
        if self.use_vae:
            ...
            style_img = self.preprocess_image(styimg)
        ...
        # ~ self.save_img(fake_image, "synthesized.png")

        # ~ return filename+"_synthesized.png"
        return fake_image


    # ~ def preprocess_image(self, image_file, num_channels=3, resize_method=tf.image.ResizeMethod.BICUBIC, normalize=True):
    def preprocess_image(self, image, num_channels=3, resize_method=tf.image.ResizeMethod.BICUBIC, normalize=True):
        # ~ # Load image
        ...
        if num_channels==1:
            # Reshape
            img = tf.reshape(img, [img.shape[0], img.shape[1], num_channels])
